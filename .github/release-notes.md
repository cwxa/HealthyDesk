...
- **四端产物都还没有在真机上走完 7 条验证清单**（`docs/device-matrix.md §三`）：CI 只能证明
  "能构建 + 包内容完整 + 内置后端能在 Mac 上起服务"，**权限授权、系统通知、切后台、摄像头实际效果
  都不在自动化的射程内**。Android 最后一次真机验证是 v1.3.4，Windows 只做过包级核验，
  macOS 从未有人在真机跑过。遇到问题欢迎提 Issue
...
🔴 发布前必查：
0. **正文里的 `<版本>` 占位必须全部替换成真实版本号** —— CD 只把本文件当模板贴上去，**不会替换**。
   核验方法：`gh api repos/<owner>/<repo>/releases/<id> --jq '.body' | grep '<版本>'` 应为空。
0b. 若**真机门未满足仍要放行**（可以放，但措辞必须如实），在 `docs/device-matrix.md §4.3`
   留一行放行记录：写清决定、放行范围、对外措辞。**「已发布」不能被读成「已验证」。**
1. 本 Release 各端产物**是否都做过真机验证**？CI 只证明"可编译、可打包"，不等于"真机跑通"。
   没验过的端，要么别发，要么在正文"已知限制"里如实标注 —— 且**不许说"可用"**。
