...
> **归档资料 · 仅历史参考**。这是立项时（v1.0 之前）用于生成整个项目的原始提示词，
> 原文件名为根目录的 `doc.md`，2026-09-24 移入 `docs/archive/`。
>
> ⚠️ 它描述的是**当时的设想**——单机 Windows 桌面软件，后端负责姿态识别与定时提醒。
> 项目最终演化为**四端**（Windows / macOS / Android / iOS），移动端**没有后端**，
> 姿态检测与调度全部下沉前端本地；评分算法也经过多轮重写。
> **不要照本文档实现**：当前架构 / 目录 / 数据库 / API 以 [`../DEVELOPMENT.md`](../DEVELOPMENT.md) 为准。
>
> 保留它的价值在于"当初想要什么"：需求边界、评分维度、UI 规范、隐私原则这些**意图**
> 至今仍成立，可作设计取舍的参照。下文目录树里的 `doc.md` 即本文件。
